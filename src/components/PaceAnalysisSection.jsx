  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-8 text-white shadow-xl space-y-10">
      <div className="space-y-4">
        <p className="text-xs uppercase tracking-[0.4em] text-teal-300">PACE L2 Analysis</p>
        <h2 className="text-2xl md:text-3xl font-semibold">Interactive PACE OCI Explorer</h2>
        <p className="text-sm text-white/70 leading-relaxed max-w-3xl">
          Interact with the heatmaps and spectra produced from the September 2025 PACE granules. Zoom, pan, and export plots generated by
          the Python workflows in <code className="rounded bg-white/10 px-1">plots/first</code> and <code className="rounded bg-white/10 px-1">plots/second</code>.
        </p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        {["nflh_05Sep", "nflh_09Sep", "avw_05Sep", "avw_09Sep"].map((key) => (
          <HeatmapCard
            key={key}
            grid={grids[key]}
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        ...
        <div className="rounded-2xl border border-white/10 bg-black/40 p-5 space-y-3">
          ...
          <ol className="space-y-2 text-sm">
            {hotspots?.map((spot) => (
              <li key={`${spot.rank}-${spot.row}-${spot.col}`} className="rounded-xl border border-white/10 bg-black/60 px-3 py-2">
                <span className="text-teal-200 mr-2">#{spot.rank}</span>
                Row {spot.row}, Col {spot.col} - Delta {spot.value.toFixed(3)}
              </li>
            ))}
          </ol>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {["chlorophyll-2025-09-01AND2025-09-07", "chlorophyll-2025-09-08AND2025-09-14"].map((key) => (
          <HeatmapCard
            key={key}
            grid={grids[key]}
            title={gridMeta[key]?.title}
