...
const DATASETS = [
  ...
  { key: "chlorophyll", code: "MY1DMM_CHLORA", source: "NASA NEO (MODIS-Aqua Chlorophyll-a)" },
];
  ...
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, (response) => {
        ...
        fileStream.on("finish", () => fileStream.close(() => resolve(destination)));
      })
      .on("error", (error) => {
...
async function ensureCsv(datasetCode, month) {
  ...
  if (!fs.existsSync(csvPath)) {
    ...
    await streamPipeline(fs.createReadStream(gzPath), createGunzip(), fs.createWriteStream(csvPath));
  }
...
function computeMonthCells(sstGrid, chlGrid, month) {
  ...
  for (let r = 0; r < rows; r += 1) {
    for (let c = 0; c < cols; c += 1) {
      ...
      cells.push({ r, c, lat: Number(lat.toFixed(3)), lon: Number(lon.toFixed(3)), sst, chl });
    }
  }
  ...
  const sstMean = Number((sstValues.reduce((sum, v) => sum + v, 0) / sstValues.length).toFixed(3));
  const chlMean = Number((chlValues.reduce((sum, v) => sum + v, 0) / chlValues.length).toFixed(3));
  const frontMean = Number((frontValues.reduce((sum, v) => sum + v, 0) / frontValues.length).toFixed(4));
  const activityMean = Number((validCells.reduce((sum, cell) => sum + cell.activity, 0) / validCells.length).toFixed(3));
  ...
  monthStats.hotspotCount = validCells.filter((cell) => cell.activity >= 0.7).length;
  return monthStats;
}
...
function percentile(values, target) {
  ...
  const index = Math.min(sorted.length - 1, Math.max(0, Math.round(target * (sorted.length - 1))));
  return sorted[index];
}

function buildScenarioDescription(id, stats) {
  if (id === "gulf_stream_warm") {
    return `Peak thermal pulse with average SST ${stats.sstMean.toFixed(1)} degC and boosted activity index ${stats.activityMean.toFixed(2)}.`;
  }
  if (id === "gulf_stream_night") {
    return `Maximum frontal shear (${stats.frontMean.toFixed(2)} gradient units) drives nocturnal predator excursions.`;
  }
  if (id === "gulf_stream_relax") {
    return `Surface bloom relaxation; chlorophyll falls to ${stats.chlMean.toFixed(2)} mg/m^3 and activity eases.`;
  }
  return `Representative baseline month with SST ${stats.sstMean.toFixed(1)} degC and balanced productivity.`;
}
...
function buildLayers(monthData, scenarioId, sourceInfo) {
  ...
  const topActivity = [...monthData.cells]
    ...
    .map((cell, index) => ({
      ...
      lat: cell.lat,
      lng: cell.lon,
      confidence: Number(Math.min(0.99, Math.max(0.1, cell.activity)).toFixed(2)),
      dietSignal: `Energy index +${Math.round((cell.activity - 0.5) * 100)}%`,
      supportingDrivers: [
...
function buildDietBreakdown(monthsData) {
  ...
  const frontTotal = monthsData.reduce((sum, month) => sum + month.frontMean, 0);
...
function buildCorrelationMatrix(points) {
  ...
  const mean = Object.fromEntries(variables.map((variable) => [variable, average(array.map((item) => item[variable]))]));

  const covariance = (a, b) => average(array.map((item) => (item[a] - mean[a]) * (item[b] - mean[b])));
  const variance = Object.fromEntries(variables.map((variable) => [variable, covariance(variable, variable)]));
...
function sampleScatterPoints(monthsData, limit = 800) {
  const merged = monthsData.flatMap((month) => month.cells.map((cell) => ({
    lat: cell.lat,
    lon: cell.lon,
  ...
  const sampled = merged.filter((_, index) => index % step === 0).slice(0, limit);
  const threshold = percentile(sampled.map((point) => point.activity), 0.9);
...
function buildSummaryStats(monthStats, referenceStats) {
  return {
    sharkOccurrence: {
      ...
      delta: Number((monthStats.activityMean - (referenceStats?.activityMean ?? monthStats.activityMean)).toFixed(3)),
    },
    hotspotsFound: {
      value: monthStats.hotspotCount,
      delta: monthStats.hotspotCount - (referenceStats?.hotspotCount ?? monthStats.hotspotCount),
    },
    avgTemperature: {
      value: Number(monthStats.sstMean.toFixed(2)),
      delta: Number((monthStats.sstMean - (referenceStats?.sstMean ?? monthStats.sstMean)).toFixed(2)),
    },
    phytoplanktonLevel: {
      label: labelForChl(monthStats.chlMean),
      delta: Number((monthStats.chlMean - (referenceStats?.chlMean ?? monthStats.chlMean)).toFixed(3)),
    },
  };
}

function buildSimulationTimeline(scenarios, analyticsSeries, referenceStats, sourceInfo) {
...
(async () => {
  try {
    ...
    const chlDataset = DATASETS.find((dataset) => dataset.key === "chlorophyll");

    for (const month of months) {
      const sstPath = path.join(RAW_DIR, sstDataset.code, `${sstDataset.code}_${month}.csv`);
      const chlPath = path.join(RAW_DIR, chlDataset.code, `${chlDataset.code}_${month}.csv`);
    ...
    const baselineData = [...monthObjects].sort((a, b) => a.activityMean - b.activityMean)[Math.floor(monthObjects.length / 2)];
    const warmData = [...monthObjects].sort((a, b) => b.sstMean - a.sstMean)[0];
    const frontData = [...monthObjects].sort((a, b) => b.frontMean - a.frontMean)[0];
    const relaxData = [...monthObjects].sort((a, b) => a.chlMean - b.chlMean)[0];

    const scenarios = [
      { id: "gulf_stream", label: "Gulf Stream baseline", data: baselineData },
      { id: "gulf_stream_warm", label: "Warm eddy pulse", data: warmData },
      { id: "gulf_stream_night", label: "Night migration", data: frontData },
      { id: "gulf_stream_relax", label: "Bloom relaxation", data: relaxData },
    ];
    ...
    const simulationTimeline = buildSimulationTimeline(scenarios, analyticsSeries, baselineData, sourceInfo);
...
export const oceanLayers = ${JSON.stringify(buildLayers(baselineData, "gulf_stream", sourceInfo), null, 2)};

export const analyticsSeries = ${JSON.stringify(analyticsSeries, null, 2)};

export const summaryStats = ${JSON.stringify(buildSummaryStats(baselineData, baselineData), null, 2)};
...
export const seasonalSeriesByRegion = ${JSON.stringify(seasonalByRegion, null, 2)};

export const correlationMatricesByRegion = ${JSON.stringify(correlationsByRegion, null, 2)};

export const scatterCloudsByRegion = ${JSON.stringify(scatterByRegion, null, 2)};

export const insightRegions = ${JSON.stringify(insights, null, 2)};

export const simulationTimeline = ${JSON.stringify(simulationTimeline, null, 2)};
`;
